...
const getNotifications = async (req, res) => {
    try {
        const notifications = await Notification.find({
            ...
            .populate(
                "fromUser",
                "name role companyName universityName"
            )
        ...
        for (const notification of notifications) {
            ...
            if (notification.type === "message") {
                const senderName =
                    notification.fromUser?.companyName ||
                    notification.fromUser?.universityName ||
            ...
            if (notification.type === "application") {
                const parsedTitleOpportunity =
                    parseOpportunityNameFromTitle(notification.title);

                const opportunityName =
                    await getOpportunityName(
                        notification.relatedApplication
                    ) ||
                    parsedTitleOpportunity ||
                    "Application";
                ...
                if (lowerTitle.includes("accepted")) {
                    ...
                    lowerTitle.includes("withdrawn")
                ) {
                    statusText = "withdrawn";
                }

                if (statusText === "withdrawn") {
                    enrichedNotifications.push({
                        ...plain,
                        opportunityName,
                        displayTitle: `You withdrew your application for ${opportunityName}`,
                        displayMessage: `You withdrew your application for ${opportunityName}.`
                    });
                } else {
                    enrichedNotifications.push({
                        ...plain,
