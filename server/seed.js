...
const runSeed = async () => {
    try {
        ...
        for (const circular of circularPosts) {
            const author = seededProfiles.find(
                (profile) => profile.email === circular.email
            );
        ...
        console.log(
            `Ensured ${demoProfiles.length} searchable company/university profiles and circular posts exist`
        );
